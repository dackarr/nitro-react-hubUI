...
import { AutoGrid, Button, Column, Flex, LayoutCurrencyIcon, LayoutGridItem, LayoutLoadingSpinnerView, Text } from '../../../../../common';
...
export const CatalogLayoutVipBuyView: FC<CatalogLayoutProps> = props =>
{
    ...
    const purchaseSubscription = useCallback((offer: ClubOfferData) =>
    {
        if(!offer) return;

        setPurchaseState(CatalogPurchaseState.PURCHASE);
        SendMessageComposer(new PurchaseFromCatalogComposer(currentPage.pageId, offer.offerId, null, 1));
    }, [ currentPage ]);
    ...
    const getPurchaseButton = useCallback((offer: ClubOfferData) =>
    {
        ...
        {
            return <Button fullWidth variant="danger">{ LocalizeText('catalog.alert.notenough.activitypoints.title.' + offer.priceActivityPointsType) }</Button>;
        }

        if(pendingOffer !== offer)
        {
            return <Button fullWidth variant="success" onClick={ (event) => { event.stopPropagation(); setOffer(offer); setPurchaseState(CatalogPurchaseState.CONFIRM); } }>{ LocalizeText('buy') }</Button>;
        }

        switch(purchaseState)
        {
            case CatalogPurchaseState.CONFIRM:
                return <Button fullWidth variant="warning" onClick={ () => purchaseSubscription(offer) }>{ LocalizeText('catalog.marketplace.confirm_title') }</Button>;
    ...
    }, [ pendingOffer, purchaseState, purchaseSubscription, getCurrencyAmount, setOffer ]);
    ...
    return (
        <Column fullHeight justifyContent='end'>
            <Column fullHeight size={ 5 } overflow="hidden">
                <Column fullHeight center overflow="hidden">
                    ...
                    <Text center overflow="hidden" dangerouslySetInnerHTML={ { __html: getSubscriptionDetails } } />
                </Column>
            </Column>
            <Column fullHeight size={ 7 } overflow="hidden" justifyContent="around">
                <AutoGrid columnCount={ 1 } className="nitro-catalog-layout-vip-buy-grid">
                    ...
                    {
                        return (
                            <LayoutGridItem key={ index } column={ false } center={ false } alignItems="center" justifyContent="between" itemActive={ pendingOffer === offer } className="p-1">
                                <Column fullWidth gap={1} justifyContent='around'>
                                    <Flex fullWidth justifyContent='center' className='title'>
