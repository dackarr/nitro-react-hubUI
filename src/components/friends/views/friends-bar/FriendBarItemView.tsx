...
export const FriendBarItemView: FC<{ friend: MessengerFriend }> = props =>
{
    ...
    {
        return (
            <div ref={ elementRef } className="btn btn-hubGrey1 friend-bar-item friend-bar-search">
    ...
    return (
        <div ref={ elementRef } className={ 'friend-bar-item ' + (isVisible ? 'friend-bar-item-active' : '') } onClick={ event => setVisible(prevValue => !prevValue) }>
            <div className={ `friend-bar-item-head position-absolute ${ friend.id > 0 ? 'avatar': 'group' }` }>
                { (friend.id > 0) && <LayoutAvatarImageView className={ `${ isVisible ? 'avatar-expanded' : 'avatar-no-expanded' }` } headOnly={ false } figure={ friend.figure } direction={ 3 } /> }
