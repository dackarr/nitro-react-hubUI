...
export const InventoryFurnitureItemView: FC<{ groupItem: GroupItem }> = props =>
{
    ...
    return <LayoutGridItem className={ `layout-inner ${ !count ? 'opacity-0-5 ' : '' }` } itemImage={ groupItem.iconUrl } itemCount={ groupItem.getUnlockedCount() } itemActive={ (groupItem === selectedItem) } itemUniqueNumber={ groupItem.stuffData.uniqueNumber } itemUnseen={ groupItem.hasUnseenItems } onMouseDown={ onMouseEvent } onMouseUp={ onMouseEvent } onMouseOut={ onMouseEvent } onDoubleClick={ onMouseEvent } { ...rest } />;
}
