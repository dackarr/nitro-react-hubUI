...
export const InventoryBotItemView: FC<PropsWithChildren<{ botItem: IBotItem }>> = props =>
{
    ...
    return (
        <LayoutGridItem className="align-items-center justify-content-center flex-column layout-inner" style={ { backgroundPosition: 'center center', backgroundRepeat: 'no-repeat' } } itemActive={ (selectedBot === botItem) } itemUnseen={ unseen } onMouseDown={ onMouseEvent } onMouseUp={ onMouseEvent } onMouseOut={ onMouseEvent } onDoubleClick={ onMouseEvent } { ...rest }>
            <LayoutAvatarImageView style={ { backgroundPosition: '-1px -30px' } } figure={ botItem.botData.figure } direction={ 3 } headOnly={ true } />
            { children }
        </LayoutGridItem>
    );
}
